...
def plotLambdas_Val_Spac(specDataList, yscale='log',vm=None):
	doEnergy = True if vm is not None else False
...
def plotLambdasBySpec(specDataList, yscale='log',vm=None):
	doEnergy = True if vm is not None else False
	if not isinstance(specDataList, list):
		specDataList = [specDataList]
	specPlotList = []
