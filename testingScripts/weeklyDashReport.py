...
def main():
    ...
    try:
        ...
        print(
            'ERROR: Unable to open jobs.txt for weekly dash.\n'
    ...
    # Read UT date/time from the test results.
    if verbose:
        print('Reading UT from weekly dash output.')
    cmd = (
    ...
    try:
        ...
        print(
            'ERROR: Unable to read performance data for weekly dash.\n'
            f"e.cmd = {e.cmd}\n"
    ...
    # Read RT from the test results.
    if verbose:
        print('Reading RT from weekly dash output.')
    cmd = (
    ...
    try:
        ...
        print(
            'ERROR: Unable to read RT for weekly dash.\n'
            f"e.cmd = {e.cmd}\n"
            f"e.returncode = {e.returncode}\n"
            'See testing log for output.\n',
            file=sys.stderr
    ...
    # Convert UT from str to datetime.
    ...
    if verbose:
        print(f"Reading simulation results from {VOLTRON_OUTPUT_FILE}.")

    # Read simulation data.
    if verbose:
        print(f"Reading Dst and CPCP data from {VOLTRON_OUTPUT_FILE}.")
    nSteps, sIds = kh5.cntSteps(VOLTRON_OUTPUT_FILE)
    symh = kh5.getTs(VOLTRON_OUTPUT_FILE, sIds, 'SymH')
    MJD = kh5.getTs(VOLTRON_OUTPUT_FILE, sIds, 'MJD')
    BSDst = kh5.getTs(VOLTRON_OUTPUT_FILE, sIds, 'BSDst')
    nCPCP = kh5.getTs(VOLTRON_OUTPUT_FILE, sIds, 'cpcpN')
    sCPCP = kh5.getTs(VOLTRON_OUTPUT_FILE, sIds, 'cpcpS')
    UT = Time(MJD, format='mjd').isot
    ut_datetime = [datetime.datetime.strptime(ut, '%Y-%m-%dT%H:%M:%S.%f')
                   for ut in UT] # needed to plot symh
    # if debug:
...
#         devcurrentGitHash = git_hash
    ...
    #     masterUTdt = [datetime.datetime.strptime(ut, '%Y-%m-%d %H:%M:%S')
    #                   for ut in masterUT]
...
#         devcurrentUTsimdt = [datetime.datetime.strptime(ut, '%Y-%m-%dT%H:%M:%S.%f') for ut in devcurrentUTsim]
    ...
    # <HACK>
    # UTdt = [datetime.datetime.strptime(ut, '%Y-%m-%d %H:%M:%S')
    #         for ut in utData]
    # RT = rtData_f
    # </HACK>
...
#         ax.plot(devcurrentUTdt, devcurrentRT, label=f"dev current ({devcurrentGitHash})", linewidth=line_width)
    ax.plot(utc_dt, rtData_f, label=f"Latest ({git_hash})",
...
#         print(f"weekly run completed successfully on branch {git_branch_name}")
    ...
    if be_loud:
        common.slack_send_image(slack_client, 'perfPlots.png',
                                is_test=is_test)
