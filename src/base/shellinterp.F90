...
module shellinterp
    ...
    type ShellGrid_T
        integer :: Np,Nt !nNumber of lon/lat cells (phi/theta for those of us who are not geophysicists)
        ! xxc = centers (Nx)
        ! xx  = corners (Nx+1)
        ! th (theta) runs from north pole toward south
        ! Assuming lat \in -pi/2,pi/2 and lon \in [0,2pi]
        ! note, th/ph are colatitude/longitude; also, defining lat/latc for geophysicists
