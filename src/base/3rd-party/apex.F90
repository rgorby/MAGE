...
!
! 2025 May: M. Wiltberger (NCAR/HAO) added the ability to read the IGRF coefficients
!   from a file.  The file name is passed to the subroutine read_igrf.  
!   Using the coefficents will be come the default in the future.
...
! Local:
  ...
  if (bhor == 0.) then
! To avoid dividing by zero, alon and a are set here when bhor=0.
    alon = 0.
    a = 1.e34
    return
  endif
...
! Based on code written by Wally Clark, NOAA, 1973.
! Modified by A.D. Richmond, 1994.
! Modified by R. Barnes, 2004, to repair problem noted by Dan Weimer.
!  where the apex location produced by fint may still have a non-zero
...
! Local:
  ...
  !! Set up the coefficients for the IGRF model
  !! The coefficients are read from a file, or hard-coded for the
  !! IGRF-14 model. The file is expected to be in the format
  !! used by the IGRF model coming from the IGRF website.
  !! https://www.ngdc.noaa.gov/IAGA/vmod/coeffs/igrf14coeffs.txt
  !! The default is to use the hard-coded values for IGRF-14, 
  !! but that will change when the coefficients are updated by NGDC.
  if (igrf14coeffs_read) then
    !print *, 'Reading IGRF coefficients from file...'
    call read_igrf("igrf14coeffs.txt", matrix)
    do col = 1, ncn1
      g1(:,col) = matrix(1:n1,col)
    enddo
    do col = ncn1+1, ncn1+ncn2
      g2(:,col-ncn1) = matrix(1:n2,col)
    enddo
  else
      g1(:,1) = (/                                               & ! 1900
      ...
      g2(:,6) = (/                                               & ! 2020
      -29403.41,-1451.37, 4653.35,-2499.78, 2981.96,-2991.72,   &
        1676.85, -734.62, 1363.00,-2380.80,  -81.96, 1236.06,   &
          241.80,  525.60, -542.52,  902.82,  809.47,  282.10,   &
          86.18, -158.50, -309.47,  199.75,   47.44, -350.30,   &
        -234.42,  363.26,   47.52,  187.86,  208.36, -140.73,   &
        -121.43, -151.16,   32.09,   13.98,   99.14,   65.97,   &
          65.56,  -19.22,   72.96,   25.02, -121.57,   52.76,   &
          -36.06,  -64.40,   13.60,    8.96,  -64.80,   68.04,   &
          80.54,  -76.63,  -51.50,   -8.23,  -16.85,   56.45,   &
            2.36,   15.80,   23.56,    6.30,   -2.19,   -7.21,   &
          -27.19,    9.77,   -1.90,   23.66,    9.74,    8.43,   &
          -17.49,  -15.23,   -0.49,   12.83,  -21.07,  -11.76,   &
          15.28,   14.94,   13.65,    3.62,  -16.59,   -6.90,   &
          -0.34,    2.90,    5.03,    8.36,  -23.44,    2.84,   &
          11.04,   -1.48,    9.86,   -1.14,   -5.13,  -13.22,   &
          -6.20,    1.08,    7.79,    8.82,    0.40,   -9.23,   &
          -1.44,  -11.86,    9.60,   -1.84,   -6.25,    3.38,   &
          -0.11,   -0.18,    1.66,    3.50,   -0.86,    4.86,   &
            0.65,   -8.62,   -0.88,   -0.11,    1.88,   -4.26,   &
            1.44,   -3.43,   -2.38,   -0.10,   -3.84,   -8.84,   &
            2.96,   -1.36,   -0.02,   -2.51,    2.50,    2.31,   &
          -0.55,   -0.85,   -0.39,    0.28,    0.62,   -0.66,   &
          -0.21,   -0.07,   -1.66,    1.44,   -1.60,   -0.59,   &
          -2.98,    0.18,   -1.97,    3.09,   -2.51,   -2.00,   &
          -0.13,   -1.15,    0.43,    0.52,    1.28,    1.37,   &
          -1.14,   -1.81,    0.71,    0.08,    0.31,    0.71,   &
            0.49,   -0.15,   -0.26,    0.55,   -0.47,    0.16,   &
            0.09,   -0.93,   -1.13,   -0.04,   -0.33,    0.52,   &
            0.08,   -0.93,   -0.88,    0.53,    0.64,    0.72,   &
            1.40,   -0.30,   -0.38,    0.75,   -1.31,   -0.01,   &
          -0.09,    0.76,    0.29,   -0.05,   -0.11,    0.37,   &
            0.47,    0.13,    0.54,    0.45,   -0.41,   -0.46,   &
          -0.36,   -0.40,   -0.60/)
...
!-----------------------------------------------------------------------
subroutine solgmlon(gclat,gclon,colat,elon,mlon)
!
! Compute geomagnetic longitude of the point with geocentric spherical
!  latitude and longitude of gclat and gclon, respectively.
...
!  Compute cosine of solar zenith angle from geographic coordinates of
!  point in question and of subsolar point.  Note that the zenith angle
!  is the angle between the vertical vector at the point in question
!  and the vertical vector at the subsolar point, and these vectors are
!  described by geographic, not geocentric, coordinates.
!
!  INPUTS:
!    glat    = geographic latitude of point (degrees)
!    glon    = geographic longitude of point (degrees)
!    sbsllat = geographic latitude of subsolar point (degrees)
!    sbsllon = geographic longitude of subsolar point (degrees)
!  OUTPUT:
...
! Local:
  ...
  stc = cos(gclat*dtr)
  ctc = sin(gclat*dtr)
  ctm = ctp*ctc + stp*stc*cos((gclon-elon)*dtr)
!
! elon is in module data, and was set by cofrm (called from apex_mka)
! ctp,stp are in module data
! dtr, rtd are in module data
!
  ctm = min(ctm,1.)
  ctm = max(ctm,-1.)
  gmlat = asin(ctm)*rtd
  gmlon = atan2(stp*stc*sin((gclon-elon)*dtr),ctp*ctm-ctc)*rtd
!
!

end subroutine gc2gm
!-----------------------------------------------------------------------
subroutine read_igrf(filename, coeffs)
  ...
  ! === Skip comments and find the year line ===
  row = 1
  header_rows = 4
  do while (row <= header_rows)
  ...
  do while (row <= max_rows)
    read(unit, '(A)', iostat=ios) line
    read(line, *) dummy_type, dummy1, dummy2, values
    coeffs(row - header_rows, :) = values
    row = row + 1
  end do
