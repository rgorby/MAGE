...
!      end associate
   ...
   subroutine CoupleIMagToMix(vApp)
      ...
      integer :: n,i,j,k,Nk
      real(rp) :: x1,x2,x1c,x2c,di,dj,dk,t
      logical :: isTasty
      ...
      associate(G=>vApp%remixApp%ion(NORTH)%G, ion=>vApp%remixApp%ion)
         ...
         !$OMP private(i,j,k,x1,x2,x1c,x2c,imP,isTasty)
            do j=1,G%Np
               do i=1,G%Nt
                  ! NH mapping
                  ...
                  ! Only interpolate if x1c is within the grid.
                  ...
                  ! x1: SH 
                  if(x1c>65.0_rp*PI/180.0_rp .and. imP(RAI_NPSP )>10.0_rp*1.0e6 .and. imP(RAI_GTYPE)<0.5) then
                     write(*,"(2(a,i4),3(a,f8.1),2(a,e12.4))") 'ldong_20250512 i=',i,' j=',j,' x1c=',x1c*180.0_rp/PI,' x2c=',x2c*180.0_rp/PI, &
                     ' gtype=',imP(RAI_GTYPE),' npsp=',imP(RAI_NPSP),' NH npsp=',ion(NORTH)%St%Vars(j,i,IM_NPSP )
                  endif
               enddo !i
            enddo !j
      end associate
