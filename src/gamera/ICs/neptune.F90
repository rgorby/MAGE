...
!---
    ...
    subroutine CoroInner(bc,Model,Grid,State)
        ...
        integer :: i,j,k,ig,ip,n
        real(rp), dimension(NVAR) :: pCon,pW,gCon,gW
        real(rp) :: xc,yc,zc
        real(rp), dimension(NDIM) :: Bd

        !i-boundaries (IN)
        !$OMP PARALLEL DO default(shared) &
        !$OMP private(i,j,k,ig,ip,n,xc,yc,zc) &
        !$OMP private(pCon,pW,gCon,gW,Bd)        
        do k=Grid%ksg,Grid%keg
            do j=Grid%jsg,Grid%jeg
                ...
                do n=1,Model%Ng
                    ...
                    !Get matching physical cell
                    pCon = State%Gas(ip,j,k,:,BLK)
                    call CellC2P(Model,pCon,pW)

                    !Create ghost primitives
                    ...
                    gW(VELX:VELZ) = Om0*[-yc,xc,0.0_rp]
                    
                    !Map back to con and put back in
                    call CellP2C(Model,gW,gCon)
                    State%Gas(ig,j,k,:,BLK) = gCon

                    !Now do magnetic field stuff
                    call cellCenter(Grid,Grid%is,j ,k ,xc,yc,zc)
                    call TiltDipole(xc,yc,zc,Model%t,Bd(XDIR),Bd(YDIR),Bd(ZDIR))
                    State%Bxyz(ig,j,k,:) = Bd

                    State%magFlux(ig,j,k,IDIR) = Grid%face(ig,j,k,IDIR)*dot_product(Bd,Grid%Tf(ig,j,k,NORMX:NORMZ,IDIR))
                    State%magFlux(ig,j,k,JDIR) = Grid%face(ig,j,k,JDIR)*dot_product(Bd,Grid%Tf(ig,j,k,NORMX:NORMZ,JDIR))
                    State%magFlux(ig,j,k,KDIR) = Grid%face(ig,j,k,KDIR)*dot_product(Bd,Grid%Tf(ig,j,k,NORMX:NORMZ,KDIR))
...
!---
    ...
    !                 call TiltDipole(xc,yc,zc,Model%t,Bd(XDIR),Bd(YDIR),Bd(ZDIR))
    !                 State%Bxyz(ig,j,k,:) = Bd
    !                 State%magFlux(ig,j,k,IDIR) = State%magFlux(Grid%is,jp,kp,IDIR)
    !                 State%magFlux(ig,j,k,JDIR) = State%magFlux(Grid%is,jp,kp,JDIR)
    !                 State%magFlux(ig,j,k,KDIR) = State%magFlux(Grid%is,jp,kp,KDIR)
