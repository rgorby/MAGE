...
module gamapp_mpi
    ...
    type, extends(GamApp_T) :: gamAppMpi_T
        ...
        integer(kind=MPI_MYADDRESS), dimension(:), allocatable :: sendDisplsGas, recvDisplsGas
        ...
        integer(kind=MPI_MYADDRESS), dimension(:), allocatable :: sendDisplsMagFlux, recvDisplsMagFlux
    ...
    subroutine calcDatatypeFC(gamAppMpi,iData,jData,kData,dType,doFace,doEdge,doCorner)
        ...
        integer(kind=MPI_MYADDRESS) :: offsetArray(3)
        ...
        countArray = (/1,1,1/)
        offsetArray = (/integer(kind=MPI_MYADDRESS):: offsetI, offsetJ, offsetK /)
        if(sendSharedFace) then
    ...
    subroutine appendDatatype(appendType,dType,offset)
        ...
        integer :: ierr
        integer(kind=MPI_MYADDRESS) :: tempOffsets(2)
        ...
        if(appendType == MPI_DATATYPE_NULL) then
            ...
            ! need to use a temporary array so that the ints are of type MPI_MYADDRESS
