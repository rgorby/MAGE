...
!     On open field lines, values of ETA will be zero:
      ...
      write(*,*) 'after Read_alam, kcsize, fudgec(1)',kcsize, fudgec(1)
      IF (ierr < 0) RETURN
      ...
      IF (rec == 1) THEN
       ...
       ! set the plasmasphere with a refilling model   sbao 03/28
         call set_plasmasphere(idim,jdim,kdim,xmin,ymin,vm,eeta_new,imin_j)
...
!      end do
        ...
        do i=imin_j(j),idim
                ...
                eeta(i,j,1) = dens_gal/(1.5695e-16*vm(i,j)**1.5)
                end if
        end do
        end do
