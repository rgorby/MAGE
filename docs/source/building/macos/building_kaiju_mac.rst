...
Building the ``kaiju`` software on a Mac running MacOS Ventura
==================================================================

Introduction
------------

These instructions will walk you through the process of building and installing the ``kaiju`` software on a Macintosh computrer running the MacOS Ventura operating system.
...
The ``kaiju`` build instructions have been written in a platform-independent manner as much as possible. In order to duplicate the directory structure used on the ``cheyenne`` and ``derecho`` systems (which share the same home directory for each user), the Mac build uses a ``ventura`` subdirectory.
...
#. 
   Set the environment variables for HDF5 (adjust the paths as needed for your machine):
