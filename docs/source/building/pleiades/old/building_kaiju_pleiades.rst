...
NOTE: In the commands shown below, the shell prompt (the strings starting with ``ewinter@pfe26:``\ ) are provided to distinguish commands from their outputs. In your session, your prompt may be different.

In the instructions below, all code will be built and installed in a ``pleiades``\ -specific subdirectory of the user home directory, i.e. ``$HOME/pleiades``. This particular organization is not required - it is used here merely to mirror the system used on ``cheyenne`` and ``derecho`` for segregating builds for different systems.
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~> module purge
   ewinter@pfe26:~> module load nas
   ewinter@pfe26:~> module load pkgsrc/2022Q1-rome  # For git-lfs and cmake
   ewinter@pfe26:~> module load comp-intel/2020.4.304  # Latest version
   ewinter@pfe26:~> module load hdf5/1.8.18_serial
...
Create a ``pleiades``\ -specific build directory.

.. code-block::

   #!shell
   ewinter@pfe26:~> cd $HOME
   ewinter@pfe26:~> mkdir pleiades
   ewinter@pfe26:~> cd pleiades
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades> mkdir development
   ewinter@pfe26:~/pleiades> cd development
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development> git clone git@bitbucket.org:aplkaiju/kaiju.git
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development> ls kaiju
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development> cd kaiju
   ewinter@pfe26:~/pleiades/development/kaiju> git switch development
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development/kaiju> mkdir build_serial
   ewinter@pfe26:~/pleiades/development/kaiju> cd build_serial
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development/kaiju/build_serial> date; time FC=`which ifort` cmake -DALLOW_INVALID_COMPILERS=ON .. >& cmake.out; date
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development/kaiju/build_serial> date; time make >& make.out; date
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development/kaiju/build_serial> ls bin
...
.. code-block::
   ...
   ewinter@pfe26:~> module load pkgsrc/2022Q1-rome  # For git-lfs and cmake
   ewinter@pfe26:~> module load comp-intel/2020.4.304  # Latest version
   ewinter@pfe26:~> module load mpi-hpe/mpt.2.23
   ewinter@pfe26:~> module load hdf5/1.8.18_mpt
...
Create a ``pleiades``\ -specific build directory (skip if already done).
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development> cd kaiju
   ewinter@pfe26:~/pleiades/development> git switch development
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development/kaiju> mkdir build_mpi
   ewinter@pfe26:~/pleiades/development/kaiju> cd build_mpi
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development/kaiju/build_mpi> date; time FC=`which ifort` cmake -DENABLE_MPI=ON -DALLOW_INVALID_COMPILERS=ON .. >& cmake.out; date
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development/kaiju/build_mpi> date; time make >& make.out; date
...
.. code-block::
   ...
   #!shell
   ewinter@pfe26:~/pleiades/development/kaiju/build_mpi> ls bin
