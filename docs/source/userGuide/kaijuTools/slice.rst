...
``slice.x`` extracts a 2D (``Z=z0`` or ``Y=y0``) slice from GAMERA output on either the
MAGE grid or interpolated onto a cartesian or a polar grid. Output is saved to an
