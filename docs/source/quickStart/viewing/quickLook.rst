...
.. code-block::
   ...
   Creates visualization of ground dB
       NOTE: Assumes ground dB has been calculated using calcdb.x on
       simulation data.
...
As an example, running ``dbpic.py`` on the output generated by the standard
``geo_mpi`` quickstart case generates the following file (\ ``qkdbpic.png``\ ):
...
``geo_mpi`` quickstart case generates the following file
(``qkdstpic.png``):
...
This script visualizes results from REMIX in a ``gamera.x`` magnetosphere run.
...
``geo_mpi`` quickstart case generates the following files
(``remix_n.png``, ``remix_s.png``) (NOTE: Images are large, so they have been
reduced in size for use on the wiki):
...
This script visualizes results from a ``gamera.x`` magnetosphere run. The
script is found at:
...
``geo_mpi`` quickstart case generates the following file (``qkpic.png``)
(NOTE: Image is large, so it has been reduced in size for use on the wiki):
...
As an example, running ``rcmhpic.py`` on the output generated by the standard
``geo_mpi`` quickstart case generates the following file (``qkrcmpic.png``)
(NOTE: Image is large, so it has been reduced in size for use on the wiki):
