...
The ``kaiju`` software is a combination of Fortran and Python code. In general, Python is used for pre- and post-processing, data analysis, and various utility functions, while Fortran executables are used as the computational core. The ``kaiju`` code is managed on `BitBucket <https://bitbucket.org/aplkaiju/kaiju>`_.
...
**Note**: Throughout these instructions, ``KAIJUHOME`` refers to the base directory of the ``kaiju`` code, created when you cloned the ``kaiju`` repository. For example, if you cloned the ``kaiju`` repository in your home directory, e.g. ``/home/username``, then ``KAIJUHOME`` would be ``/home/username/kaiju``.
...
The ``kaiju`` code is developed primarily using the Intel Fortran compiler, version 17 or later. If you are building the code on an HPC system such as ``pleiades`` or ``derecho``, then the Intel compiler is avilable using the appropriate ``module`` command. The code can also be compiled using the GNU Fortran compiler.

If you need to install the Intel Fortran compiler, it is available as a `free download from Intel <https://www.intel.com/content/www/us/en/developer/tools/oneapi/fortran-compiler.html#gs.3msomg>`_. Installation instructions can be found :doc:`here <install_intel_compilers>`.
...
The Python portion of the ``kaiju`` software uses the CartoPy package when creating satellite data comparisons. CartoPy requires the `GEOS library <https://libgeos.org/>`_. This library is typically available as an environment module on HPC systems. However, the GEOS library is not available on ``pleiades``, so ``kaiju`` code which uses GEOS will not work on ``pleiades``. On a Mac, the GEOS library can be installed using HomeBrew:
...
Once these prerequisites are in place, you can proceed to building the code.
