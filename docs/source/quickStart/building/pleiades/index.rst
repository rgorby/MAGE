...
since it is not available as a module on ``pleiades``.

Instructions for building and installing the CDF library on ``pleiades`` are
available :doc:`here <pleiades_build_cdf>`.
...
``pleiades`` are available :doc:`here <pleiades_build_python>`.
...
software on ``pleiades`` are available :doc:`here <pleiades_build_kaiju>`.
