...
This default set of modules *cannot* be used to build the ``kaiju`` software.
The correct module sets for the serial and MPI versions of ``kaiju`` will be
...
since it is not available as a module on ``derecho``.

Instructions for building and installing the CDF library on ``derecho`` are
available :doc:`here <derecho_build_cdf>`.
...
Instructions for installing python and building a python environment on
``derecho`` are available :doc:`here <derecho_build_python>`.
...
Instructions for building the serial and MPI versions of the ``kaiju``
software on ``derecho`` are available :doc:`here <derecho_build_kaiju>`.
